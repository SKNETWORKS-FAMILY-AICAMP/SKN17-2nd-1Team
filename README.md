...
따라서, 어떤 개발자가 커뮤니티를 떠날 위험이 높은지 선제적으로 파악하고, 그 원인을 데이터 기반으로 분석하는 것은 커뮤니티의 지속 가능성을 위해 매우 중요하다. 본 프로젝트는 AI가 커뮤니티 감소에 어떤 영향을 끼치는지 파악하고, 이런 문제점을 해결해 커뮤니티 사이트의 지속성을 높이고자 한다.
...
이탈 확률 예측기(XGBoost) 를 학습하고,
CatBoost로 사용자별 ‘이탈 리스크 점수(risk_score)’ 를 산출하여 상위 위험군을 뽑은 다음,
“유지(이탈하지 않은)” 사용자의 행동/기술 분포(같은 직무, 비슷한 연령)를 통계로 뽑아 ‘하지 않는 행동·기술’을 추천하고,
추천을 실행 계획(주간 루틴, 액션 체크리스트, 질문 템플릿, 저장검색 쿼리, 링크)으로 변환,
Streamlit UI로 입력을 받아 실시간 예측/맞춤 추천을 보여준다.
